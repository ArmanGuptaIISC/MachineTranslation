...
for file in os.listdir(path):
    if file.endswith('.my'):
        bfile = file
        with open(os.path.join(path, bfile),'r') as burmesefile:
            burmese.extend(burmesefile.readlines())
...
assert len(english) == len(burmese)

def prepareData(burmese, english):
    size=  len(english)
    data = []
    for i in range(size):
        if(len(burmese[i].strip().split()) > 150 or len(english[i].strip().split())> 150):continue
        data.append({
            'id': i,
            "translation": {
                "my": burmese[i].strip(),
    ...
    return dataset
bu_en_books = prepareData(burmese, english)

max_input_length = 256
max_target_length = 256

prefix = "အင်္ဂလိပ်ဘာသာသို့ ဗမာ့ဘားမားဘာသာပြန်ဆို"
